...
# Minimal get_user_data_dir replacement for inference-only deployment
def get_user_data_dir(app_name: str) -> Path:
    """Get user data directory for TTS models (respects TTS_HOME environment variable)."""
    
    # Check if TTS_HOME environment variable is set (like original trainer module)
    tts_home = os.getenv('TTS_HOME')
    XDG_DATA_HOME = os.environ.get("XDG_DATA_HOME")
    if tts_home is not None:
        # Use TTS_HOME if specified
        base_dir = Path(tts_home).expanduser().resolve()
    elif XDG_DATA_HOME is not None:
        # Use XDG_DATA_HOME if specified
        base_dir = Path(XDG_DATA_HOME).expanduser().resolve()
    else:
        # Fall back to default user data directory
        if os.name == 'nt':  # Windows
            base_dir = Path(os.getenv('APPDATA', os.path.expanduser('~'))) / 'Local'
        else:  # Unix-like
...
class ModelManager:
    ...
    def __init__(
        ...
        if output_prefix is None:
            ...
            self.output_prefix = Path(output_prefix).joinpath("tts")
        self.models_dict = {}
    ...
    def download_model(self, model_name: str) -> tuple[Path, Path | None, ModelItem]:
        ...
        if (
            model not in ["knnvc"] and "fairseq" not in model_name and "xtts" not in model_name
