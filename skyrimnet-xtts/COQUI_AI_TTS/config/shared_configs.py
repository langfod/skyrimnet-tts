...
class BaseTrainingConfig(Coqpit):
    """Minimal config for inference-only TTS deployment.
    
    Replaces the full training configuration with minimal implementation
    that provides the same interface without training dependencies.
    """

    model: str = None
    # Keep minimal attributes that might be referenced by inference code
