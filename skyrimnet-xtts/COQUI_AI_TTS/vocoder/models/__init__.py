...
from COQUI_AI_TTS.utils.generic_utils import to_camel
from COQUI_AI_TTS.vocoder.configs.shared_configs import BaseGANVocoderConfig, BaseVocoderConfig
...
def setup_model(config: BaseVocoderConfig) -> BaseVocoder:
    ...
    if "discriminator_model" in config and "generator_model" in config:
        MyModel = importlib.import_module("COQUI_AI_TTS.vocoder.models.gan")
        MyModel = getattr(MyModel, "GAN")
    else:
        MyModel = importlib.import_module("COQUI_AI_TTS.vocoder.models." + config.model.lower())
...
def setup_generator(c: BaseGANVocoderConfig):
    ...
    MyModel = importlib.import_module("COQUI_AI_TTS.vocoder.models." + c.generator_model.lower())
...
def setup_discriminator(c: BaseGANVocoderConfig):
    ...
    if "parallel_wavegan" in c.discriminator_model:
        MyModel = importlib.import_module("COQUI_AI_TTS.vocoder.models.parallel_wavegan_discriminator")
    else:
        MyModel = importlib.import_module("COQUI_AI_TTS.vocoder.models." + c.discriminator_model.lower())
